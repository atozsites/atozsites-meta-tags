...
final class ATOZSITES_Meta_Tags {
     ...
     * @var float
     */
    const ATOZSITES_REQUIRED_PHP = '5.6.3';
     ...
     * @var float
     */
    const ATOZSITES_REQUIRED_WP = '4.7';
    
    /**
     * The one and only instance of the plugin (singleton).
     *
     * @var ATOZSITES_Meta_Tags
     */
    ...
    private function check_system(){
        ...
        if (
            version_compare(PHP_VERSION, self::ATOZSITES_REQUIRED_PHP, '<') || 
            version_compare(get_bloginfo('version'), self::ATOZSITES_REQUIRED_WP, '<')
        ){

            if ( is_admin() ){          

                require_once( ABSPATH . 'wp-admin/includes/plugin.php' );
                deactivate_plugins( ATOZSITES_PLUGIN_FILE );  
                ...
                wp_die(
                    sprintf(
                        ...
                        self::ATOZSITES_REQUIRED_PHP, 
                        self::ATOZSITES_REQUIRED_WP
                    ) . 
    ...
    private function run(){
        ...
        if ( is_admin() ){
            include_once dirname(__FILE__) . '/admin/class-atozsites-admin.php';
        }else{
            include_once dirname(__FILE__) . '/class-atozsites-frontend.php';
        }
