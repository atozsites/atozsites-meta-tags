...
class ATOZSITES_Save_Tags {
    ...
    public static function save( $list_of_meta_tags, $data ){
        ...
        // validate type
        if ( ! post_type_exists($data['ATOZSITES_type']) && ! taxonomy_exists($data['ATOZSITES_type']) ){
            return;
        }

        $type = $data['ATOZSITES_id'] == 'front' ? 'frontpage' : $data['ATOZSITES_type'];
        ...
        $url_fields = [ 
            ...
            'ATOZSITES_og_video',
            'ATOZSITES_og_url',
            'ATOZSITES_twitter_image',
            'ATOZSITES_twitter_player_stream'
        ];
            ...
            foreach ( $item['fields'] as $tag => $field ){
                ...
                switch (true){
                    ...
                    case taxonomy_exists( $type ):
                        update_term_meta( $data['ATOZSITES_id'], $key, $value );

                        break;


                    case $type == 'author':
                        update_user_meta( $data['ATOZSITES_id'], $key, $value );

                        break;


                    case $type == 'frontpage':
                        update_option( 'ATOZSITES_frontpage_'. $key, $value );

                        break;


                    case post_type_exists( $type ):
                        update_post_meta( $data['ATOZSITES_id'], $key, $value );
    ...
    public static function bulk( $action, $list_of_meta_tags, $wp_object_type, $meta_tag_group ){
        ...
        $items = ATOZSITES_Retrieve_List::get_list( $wp_object_type );
                ...
                if ( $item['id'] == 'front' ){
                    ...
                    foreach ( $fields_to_update as $field ){
                        ...
                        if ( $action == 'autopilot' ){
                            ...
                            update_option( 'ATOZSITES_frontpage_'. $field, 'auto' );

                        }elseif ( $action == 'delete' ){
            
                            delete_option( 'ATOZSITES_frontpage_'. $field );                            
