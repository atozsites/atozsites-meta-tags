...
class ATOZSITES_Frontend {
    ...
    public function includes(){
        ...
        include_once 'atozsites-meta-tag-list.php';
        include_once 'class-atozsites-retrieve-info.php';
        include_once 'class-atozsites-retrieve-tags.php';

        $this->meta_tag_list = $ATOZSITES_meta_tag_list;
    ...
    public function set_html_prefix_attribute(){
        ...
        add_filter( 'language_attributes', 'ATOZSITES_add_og_html_prefix' );

        function ATOZSITES_add_og_html_prefix( $output ){
    ...
    public function process_auto_tags( $tag_to_process ){
        ...
        if ( empty($this->page_info) ){
            $this->page_info = new ATOZSITES_Retrieve_Info( $this->page_type, $this->page_id );
        }



        // general tags - we skip keywords because it's not recommended
        if( $tag_to_process == 'ATOZSITES_general_description' ){
        ...
        // open graph
        if( $tag_to_process == 'ATOZSITES_og_title' ){
            return $this->page_info->title;
        }

        if( $tag_to_process == 'ATOZSITES_og_description' ){
            return $this->page_info->description;
        }

        if( $tag_to_process == 'ATOZSITES_og_type' ){
        ...
        if( $tag_to_process == 'ATOZSITES_og_audio' ){
            return $this->page_info->audio;
        }

        if( $tag_to_process == 'ATOZSITES_og_image' ){
            return $this->page_info->image;
        }

        if( $tag_to_process == 'ATOZSITES_og_image_alt' ){
            return $this->page_info->image_alt;
        }

        if( $tag_to_process == 'ATOZSITES_og_video' ){
            return $this->page_info->video;
        }

        if( $tag_to_process == 'ATOZSITES_og_url' ){
        ...
        // twitter cards
        if( $tag_to_process == 'ATOZSITES_twitter_card' ){

            if (
                ! empty( $this->page_info->video ) &&
                ( ! empty( $this->tags['ATOZSITES_twitter_player'] ) || ! empty( $this->tags['ATOZSITES_twitter_player_stream'] ) )
            ){
                return 'player';
            }
        ...
        if( $tag_to_process == 'ATOZSITES_twitter_title' ){
            return $this->page_info->title;
        }

        if( $tag_to_process == 'ATOZSITES_twitter_description' ){
            return $this->page_info->description;
        }

        if( $tag_to_process == 'ATOZSITES_twitter_image' ){
            return $this->page_info->image;
        }

        if( $tag_to_process == 'ATOZSITES_twitter_image_alt' ){
            return $this->page_info->image_alt;
        }

        if( $tag_to_process == 'ATOZSITES_twitter_player_stream' ){
        ...
        if( $tag_to_process == 'ATOZSITES_twitter_player_stream_content_type' ){
    ...
    public function process_tags(){
        ...
        $taginfo = new ATOZSITES_Retrieve_Tags( $this->meta_tag_list );
        ...
        $ATOZSITES_autopilot_settings = get_option('ATOZSITES_autopilot_settings');
        $autopilot_arr = maybe_unserialize($ATOZSITES_autopilot_settings);
        ...
        foreach ( $this->meta_tag_list as $group => $values ){
            ...
            foreach ($values['fields'] as $field => $info) {
                ...
                if (
                    ...
                    if ( $info['variable'] == 'ATOZSITES_custom' ){
                    // user defined extra meta tags
                        ...
                        if ( ! $this->is_og_html_attr_set && substr($info['variable'], 0, 7) == 'ATOZSITES_og' ){
